...
def data_partition(fname,max_len=0):
    ...
    usernum = 0
    itemnum = 0
    User = defaultdict(list)  #创建一个字典，用于存储每个用户的购买记录
    user_train = {}
    user_valid = {}
    user_test = {}
    user_latest_time = {}
    ...
    for line in f:
        u,timestamp, i = line.rstrip().split(' ') # 读取用户编号，时间戳和商品编号
        u = int(u)
        i = int(i)
        ...
        User[u].append([i,timestamp]) # User:{user:(i,timestamp)}# 将商品和时间戳添加到该用户的购买记录中
    ...
    for user in User:
        ...
        if nfeedback < 3: # 长度小于3的，直接用作训练
            ...
            user_valid[user] = []
            user_test[user] = []
            user_latest_time[user] = User[user][-1][1]# 记录最近一次购买的时间
            user_first_time[user] = User[user][0][1]# 记录第一次购买的时间
...
def sample_function_shift_timestamp(user_train, usernum, itemnum,user_latest_time, user_first_time,batch_size, maxlen, result_queue, SEED,time_lag_first=False):
    def sample():#它负责生成单个训练样本。
        user = np.random.randint(1, usernum + 1)#随机选择一个用户，用户的编号从1到usernum之间。
        while len(user_train[user]) <= 1: user = np.random.randint(1, usernum + 1)#至少要有两条购买记录

        x_mask = np.zeros([maxlen], dtype=np.int32)  # 用来mask掉padding
        seq = np.zeros([maxlen], dtype=np.int32)#序列数据
...
#通过循环生成一个批次的样本数据，并将批次数据放入 result_queue 中，以供训练使用。不断生成样本数据，实现了数据的持续产生。
...
class WarpSampler(object):#通过多进程方式生成训练数据的批次，提高了数据生成的效率，特别适用于处理大规模数据集。
...
def evaluate(model, dataset, args,max_time_lag,time_lag_first=False):#评估模型在给定数据集上的性能，计算了多个评估指标来衡量推荐系统的效果。
